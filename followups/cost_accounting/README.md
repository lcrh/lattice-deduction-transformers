...
**Question.** The paper's training-cost columns mix hardware generations
(LDT on B200, Sotaku on H100, TRM/HRM on L40S wall-clock as reported by
their authors), which conflates method efficiency with GPU generation.
Restate every comparison in common units, and account for the offline
costs the wall-clock numbers don't show.
...
`repro/README.md`), which is exactly the same-silicon datapoint needed
for Table 3. What remains:
...
1. **GPU-hours normalization.** One table: model × task → reported
   ...
   s/step), not a hardware conversion factor. For Sudoku TRM (4× L40S,
   36h) either run the same measurement (TRM sudoku training is cheap to
...
3. **Offline α-operator / data costs.** The wall-clock tables start at
   ...
   symmetric.
4. **Inference cost column.** The per-puzzle inference costs in Table 1
   are LDT-only; add measured per-puzzle inference cost for TRM/HRM from
