...
class StepConfig:
    ...
    # Number of deduction passes (forward + threshold-eliminate) applied to
    # the SAME state before the round's single decision. Default 1 =
    # current/legacy behavior (one pass then decide), byte-identical. Values
    # > 1 iterate the learned propagator toward a lattice fixpoint (E3-O3:
    # deduce-to-fixpoint before branching). 0 = fixpoint mode: keep passing
    # until no candidate falls below threshold, capped at `deduce_pass_cap`.
    # Each pass wraps its OWN forward under its OWN fresh aug (when
    # augment=True), so iterated deduction is also an implicit test-time
    # ensemble over augmentations. Conflict / all-singleton is checked after
    # EVERY pass and breaks the loop early; the decision runs once after the
    # final pass. Inert (single pass) at the default.
    deduce_passes: int = 1
    # Safety cap for fixpoint mode (deduce_passes=0) and an upper bound on the
    # inner loop generally. Prevents an infinite loop if the operator never
    # reaches a fixpoint (e.g. oscillates). Ignored when deduce_passes==1.
    deduce_pass_cap: int = 16
    # ----- E2 decision-policy knobs (search-process ablation) -------------
    # Which multi-alive cell to branch on. Default "uniform" is byte-identical
    # to the legacy `torch.multinomial(cd_multi_alive, 1)` pick. Other policies:
    #   "mrv"          — fewest alive vocab candidates (min-remaining-values).
    #   "min_entropy"  — lowest entropy of the softmax head over alive
    #                    candidates (renormalized) — the "most decided" cell.
    #   "max_entropy"  — highest such entropy (control; should be bad).
    # Deterministic policies (mrv / *_entropy) break ties RANDOMLY (multinomial
    # over the argmin/argmax set) so they don't bias toward low cell index.
    cell_policy: str = "uniform"
    # Which candidate to pin at the chosen cell. Default "softmax" is
    # byte-identical to the legacy softmax-sample-at-temp_decide (or argmax
    # when temp_decide==0). Other policies:
    #   "argmax"  — greedy top-logit (== the temp_decide==0 path).
    #   "rank_k"  — chain k in a slot takes the k-th best digit by logit
    #               (deterministic-but-diverse). Needs the per-row `decide_rank`
    #               argument to `dpll_step`; rank is clamped to (alive_count-1)
    #               at the chosen cell. rank_k with rank 0 == argmax.
...
def dpll_step(
    ...
    *,
    ...
    Multi-pass deduction (`cfg.deduce_passes != 1`, E3-O3): the forward +
    threshold-eliminate is applied `deduce_passes` times (or to fixpoint when
    0, capped at `cfg.deduce_pass_cap`) on the SAME state before the single
    ...
    Decision policy (E2 search-process ablation, `cfg.cell_policy` /
    `cfg.digit_policy`): default ("uniform"/"softmax") is byte-identical to the
    ...
    if cfg.deduce_passes == 1:
        max_passes = 1
    elif cfg.deduce_passes == 0:
        max_passes = max(1, cfg.deduce_pass_cap)
    else:
        max_passes = cfg.deduce_passes
    ...
    # E2 (search-process ablation). Deterministic cell policies break ties
    ...
    if want_stats:
        info = {
            ...
            # Per-pass diagnostics (E3-O3 compounding analysis). At the default
            ...
            "n_passes": n_passes,
            "per_pass_deduced": (
                per_pass_deduced if cfg.deduce_passes != 1
